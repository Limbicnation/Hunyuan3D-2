...
try:
    ...
    print("WARNING: flash_attn not available. Using standard attention instead (model_cache.py).")
...
try:
    ...
    class SwiGLU(nn.Module):
        def __init__(self, dim, hidden_dim=None, bias=True, _pack_weights=False):
            super().__init__()
...
class Hourglass(torch.nn.Module):
    ...
    def __init__(self, config, vocab_size, n_heads, n_embedding, block_size, factors: List[int], dropout=0.0):
        ...
        if len(self.factors) == 2:
            ...
            self.hourglass = Hourglass(
                config, self.vocab_size, self.n_heads, self.n_embedding, math.ceil(self.block_size / self.k),
                self.factors[2:])
    ...
    def build_kv_caches(self, idx: torch.Tensor, max_seq_length: int, rope_cache_length: int, n_layer: int) -> List[
        KVCache]:
        B = idx.size(0)
    ...
    def forward(self, x,
        ...
        for i in range(self.n_layers):
            ...
            x, self.pre_kv_caches[i] = self.pre_layer[i](x, (cos, sin), max_seq_length=max_seq_len, pc=pc,
                                                         input_pos=input_pos, kv_cache=self.pre_kv_caches[i],
                                                         start=start)
        if self.hourglass is not None:
            self.post_kv_caches = self.post_kv_caches or self.build_kv_caches(x, max_seq_len, cos.size(-1) * 2,
                                                                              self.n_layers)
            x_residual = x.clone()
            ...
            if start % 3 != 0:
                ...
                x = self.hourglass(downsampled, max_seq_len=downsampled_max_seq_len, pc=pc, start=(start + 2) // self.k,
                                   window_size=downsampled_window_size, input_pos=downsampled_pos)
                x = self.upsample(x)
            ...
            for i in range(self.n_layers):
                ...
                x, self.post_kv_caches[i] = self.post_layer[i](x, (cos, sin), max_seq_length=max_seq_len, pc=pc,
                                                               input_pos=input_pos, kv_cache=self.post_kv_caches[i],
                                                               start=start)
...
class GPTCache(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                h=Hourglass(config, config.padded_vocab_size, config.n_head, config.n_embd, config.block_size,
                            self.factors),
    ...
    def _init_weights(self, module: nn.Module, n_layer) -> None:
        ...
        for name, p in module.named_parameters():
            if (name == "proj.weight" and isinstance(module, LLaMAMLP)) or (
                    name == "w3.weight" and isinstance(module, SwiGLU) or (name == "proj.weight" and isinstance(module,
                                                                                                                CausalSelfAttention))):  # if use xformer swiglu, fc2 layer will be renamed to w3
    ...
    def forward(
        ...
        x = self.transformer.h(x, max_seq_length, pc=cond_embeds, start=start, window_size=window_size,
                               input_pos=input_pos)
...
class CausalSelfAttention(nn.Module):
    ...
    def scaled_dot_product_attention(
        ...
        if (
            ...
            # Use FlashAttention if available and conditions are right
